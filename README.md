...
There are [over 40 items](https://www.wikidata.org/wiki/Property:P21#P2302) that are can be assign as a "sex or gender" (P21) to a human in Wikidata.. For this analysis however we only focus on the genders ‘male’ ([Q6581097](http://www.wikidata.org/entity/Q6581097)) and ‘female’ ([Q6581072](http://www.wikidata.org/entity/Q6581072)). Out of the 90 million name usages recorded in this dataset, 4100 were from people who had a "sex or gender" property with a value that was neither male nor female. So for the simplicity of processing, these alternative genders were grouped as "other" during the processing. They were taken into consideration when asserting the confidence of the prevalent gender of a name. 
...
We choose to limit this list to 15000 when we exported it, as this would put the cut off count at 20 occurrences per name, which is a good minimum to be able determine the prevalent gender of a name with a sufficient confidence. The query we ran was [[Query 7](#q7)].
...
|---|---|---|---|
|  "Jean"@mul <br />"Jean"@fr| 96506 | 50 | 6
...
**Data from the previous processing steps:**

"Jean"@mul: 

| Native Label  | Count Male | Count Female | Count Other |
|---|---|---|---|
|  "Jean"@mul| 96678 | 3255 | 9
...
Which is still not as good as the data we get from a properly modelled items (which in this case would be just the numbers for "Jean"@mul but this might not be the case for other inconsistently modelled names), but since we needed to automatically process 15000 names, it is sufficient for our purposes.

As the results all of the name strings with the different language tags now have the same occurrence counts, thus technically reducing the number of names in the dataset to 14808. When using this dataset to check for a given name, the user can always use the first match of the name string, knowing that all other will return the same result.

## 4 Calculating Prevalent Gender and Confidence
As the next step we determined the prevalent gender for each name. If the number of usages for males was larger than the number of usages for females plus the number of usages for other genders, the prevalent gender would be male, and the other way around [[Query 11](#q11)]. Only in 13 cases the result was inconclusive because the number of usages were so close together. But the assertion of the prevalent gender is not helpful without a score of the confidence. The calculation for such a score was the count for the prevalent gender divided by the sum of the counts for the prevalent gender, the non-prevalent gender and the other genders. 
...
# Other remarks
* In total around 7.9 million name occurrances were analysed for this data set, see [[Query 13](#q13)].
* The precise numbers used in this text and in the analysis in OpenRefine might differ from each other and from the results from live query due to the ever evolving nature of Wikidata. 
...
|---------------------------------------|-----------|----------------------|------|------|
|http://www.wikidata.org/entity/Q6581097|male       |1.00328172164779909324|251606|250783|
|http://www.wikidata.org/entity/Q6581072|female     |1.00433542653145555172|148261|147621|
...
|----------------------------------------|------------------|-----|
|http://www.wikidata.org/entity/Q12308941|male given name   |48261|
|http://www.wikidata.org/entity/Q11879590|female given name |29659|
|http://www.wikidata.org/entity/Q202444  |given name        |25409|
|http://www.wikidata.org/entity/Q1243157 |double name       |5132 |
|http://www.wikidata.org/entity/Q3409032 |unisex given name |2994 |
|http://www.wikidata.org/entity/Q22809413|Chinese given name|2801 |
...
```
SELECT ?name ?nameLabel ?class ?classLabel ?gender ?count WHERE {
  {
    SELECT ?name ?class ?gender (COUNT(?person) as ?count) WHERE {
      BIND("George"@mul as ?nameString).
      ?name wdt:P31/wdt:P279* wd:Q202444.
      ?name wdt:P31 ?class. 
      ?name wdt:P1705 ?nameString.
      #?name wdt:P1705 ?label.
      #FILTER(STR(?label) = ?nameString).
      ?person wdt:P735 ?name.
      {
...
|-------|---------|------|---------------------------------|-----------|-----|
|http://www.wikidata.org/entity/Q15921732|George   |http://www.wikidata.org/entity/Q12308941|male given name                  |male       |44680|
|http://www.wikidata.org/entity/Q15921732|George   |http://www.wikidata.org/entity/Q12308941|male given name                  |unspecified|122  |
|http://www.wikidata.org/entity/Q15921732|George   |http://www.wikidata.org/entity/Q12308941|male given name                  |female     |37   |
|http://www.wikidata.org/entity/Q15921732|George   |http://www.wikidata.org/entity/Q12308941|male given name                  |other      |4    |
|http://www.wikidata.org/entity/Q23774368|George   |http://www.wikidata.org/entity/Q18220911|male given name borne by a female|female     |15   |
|http://www.wikidata.org/entity/Q23774368|George   |http://www.wikidata.org/entity/Q18220911|male given name borne by a female|male       |4    |
...
## 11: Determine the prevalent gender <span id="q11"></span>
* create a new column "prevalent_gender" based on column "count_undefined" with the expression
```
if(cells["count_male"].value > cells["count_female"].value + cells["count_other"].value, 'male', if(cells["count_female"].value > cells["count_male"].value + cells["count_other"].value, 'female', 'inconclusive'))
```

## 12: Calculate the confidence score <span id="q12"></span>
...
```
round(cells["count_male"].value*1.0/(cells["count_male"].value + cells["count_female"].value + cells["count_other"].value)*100000)/100000.0
```
...
```
round(cells["count_female"].value*1.0/(cells["count_male"].value + cells["count_female"].value + cells["count_other"].value)*100000)/100000.0
```

## 13: Calculate the sum of all of the name occurrences in the column count in OpenRefine <span id="q13"></span>
